...
def get_plot_settings():
    ...
    samples_per_year = 12               # E.g., 12 implies 30-day samples in a 360-day year.
...
def main():
    jobs_dir = os.path.join(SCRIPT_DIR, 'plots')
    ...
    with sqlite3.connect(SIM_DB_PATH) as db:
        for eps in get_param_vals(db, 'eps'):
            for beta00 in get_param_vals(db, 'beta00'):
                for sigma01 in get_param_vals(db, 'sigma01'):
                    for sd_proc in get_param_vals(db, 'sd_proc'):
                        job_dir = os.path.join(jobs_dir, 'eps={:.2g}-beta00={:.2g}-sigma01={:.2g}-sd_proc={:.1g}'.format(
                            eps, beta00, sigma01, sd_proc
                        ))
                        os.makedirs(job_dir)
                        
                        job_ids = get_job_ids(db, eps=eps, beta00=beta00, sigma01=sigma01, sd_proc=sd_proc)[:N_REPLICATES]
                        runmany_info = OrderedDict(RUNMANY_INFO_TEMPLATE)
                        runmany_info['job_ids'] = job_ids
...
def get_param_vals(db, param_name):
    return [row[0] for row in db.execute('SELECT DISTINCT {0} FROM job_info ORDER BY {0}'.format(param_name))]

def get_job_ids(db, **kwargs):
    keys = kwargs.keys()
    values = [kwargs[key] for key in keys]
    return [row[0] for row in db.execute(
        'SELECT job_id FROM job_info WHERE {} ORDER BY job_id'.format(
            ' AND '.join(['{} = ?'.format(key) for key in keys])
        ),
        values
    )]
